...
func try_navigate(selector_index: int, direction: Vector2i) -> void:
	...
	if rect.has_point(selection[selector_index] + direction):
		...
		GameManager.player_setup[selector_index] = DataLoader.get_countries()[flag_index + 1]
...
func place_flags() -> void:
	for j in range(NB_ROWS):
		for i in range(NB_COLS):
			var flag_texture: TextureRect = TextureRect.new()
			flag_texture.position = FLAG_ANCHOR_POINT + Vector2(i * 55, j * 50)
...
func add_selector(control_scheme: Player.ControlScheme) -> void:
	...
	selector.selected.connect(on_selector_selected)
