...
@onready var world_cup_texture: TextureRect = %WorldCupTexture
@onready var stage_texture: TextureRect = %StageTexture
@onready var flags_containers: Dictionary = {
...
const STAGE_TEXTURE : Dictionary[Tournament.Stage, Texture] = {
...
func refresh_bracket_stage(stage: Tournament.Stage) -> void:
	...
	if stage < Tournament.Stage.COMPLETE:
		...
		for i in range(matches.size()):
			...
			elif [current_match.country_home, current_match.country_away].has(player_country) and \
...
func get_flag_nodes_for_stage(stage: Tournament.Stage) -> Array[BracketFlag]:
	...
	for container in flags_containers.get(stage):
