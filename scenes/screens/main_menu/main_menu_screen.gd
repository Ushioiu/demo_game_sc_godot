...
@onready var selectable_menu_nodes: Array[TextureRect] = [%SinglePlayer, %TwoPlayer]
...
func _process(_delta: float) -> void:
	if is_active:
		if KeyUtils.is_action_just_pressed(Player.ControlScheme.P1, KeyUtils.Action.UP):
			change_slected_index(current_selected_index - 1)
...
func change_slected_index(new_index: int) -> void:
...
func submit_selection() -> void:
	...
	var country_default: String = DataLoader.get_countries()[1]
