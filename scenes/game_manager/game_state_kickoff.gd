class_name GameStateKickoff extends GameState

var valid_control_scheme := []
...
func _enter_tree() -> void:
	...
	if country_starting == manager.player_setup[1]:
		valid_control_scheme.append(Player.ControlScheme.P2)
	if valid_control_scheme.size() == 0:
		valid_control_scheme.append(Player.ControlScheme.P1)

func _process(_delta: float) -> void:
	for control_scheme: Player.ControlScheme in valid_control_scheme:
