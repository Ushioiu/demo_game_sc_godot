...
func _ready() -> void:
	...
	goal_home.country = GameManager.current_match.country_home
	spawns.scale.x = -1
	kickoffs.scale.x = -1
	squad_away = spawn_players(GameManager.current_match.country_away, goal_away)
	goal_away.country = GameManager.current_match.country_away
...
func on_impact_received(impact_position: Vector2, _is_height_impact: bool) -> void:
